...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ============================================
  // LIVE SEARCH & FILTER (HISTORY TAB)
  // ============================================
  ...
  const historyVisibleCountEl = document.getElementById("historyVisibleCount");
  
  if (historySearchInput && historySortSelect && historyTableBody) {
    let historyDebounceTimer = null;
    const allHistoryRows = Array.from(historyTableBody.querySelectorAll("tr[data-driver]"));
    ...
    function filterAndSortHistory() {
      ...
      if (historyLoadingIndicator) {
        historyLoadingIndicator.classList.add("active");
      }
      
      // Filter rows
      let visibleRows = allHistoryRows.filter(row => {
        if (!query) return true;
        const driver = row.dataset.driver || "";
        const plate = row.dataset.plate || "";
        const license = row.dataset.license || "";
      ...
      visibleRows.forEach((row, index) => {
        row.style.display = "";
        const rowNumber = row.querySelector(".history-row-number");
        if (rowNumber) {
          rowNumber.textContent = index + 1;
        }
        historyTableBody.appendChild(row); // Move to end to maintain order
      });
      
      // Update count
      if (historyVisibleCountEl) {
        historyVisibleCountEl.textContent = visibleRows.length;
      }
      
      // Show empty state if no results
      let emptyRow = historyTableBody.querySelector("#historyEmptyRow");
      if (visibleRows.length === 0) {
        if (!emptyRow) {
          emptyRow = document.createElement("tr");
          emptyRow.id = "historyEmptyRow";
          emptyRow.innerHTML = `
            <td colspan="7" class="text-center text-muted py-4">
              <i class="fas fa-search fs-1 d-block mb-2 opacity-50"></i>
              No records match "${historySearchInput.value}"
            </td>
          `;
